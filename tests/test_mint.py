...
def test_mint_airdrop_should_free($name_snake_case$, accounts, admin):
    ...
    tx = $name_snake_case$.mintWithSig(ITEM1, 1, nonce, sig, {"from": accounts[1], "amount": 0})
    assert tx.return_value == True
    assert $name_snake_case$.balanceOf(accounts[2], ITEM1) == 0
...
def test_mint_invalid_signature($name_snake_case$, accounts, admin):
    with brownie.reverts("Invalid signature"):
        sig, nonce = get_mint_sig_with_nonce(accounts[1], ITEM1, 1, admin)
        $name_snake_case$.mintWithSig(ITEM1, 1, nonce, sig, {"from": accounts[3], "amount": 15 * 10 ** 17})
    with brownie.reverts("Invalid signature"):
        random_acc = accounts.add()
        sig, nonce = get_mint_sig_with_nonce(accounts[1], ITEM1, 1, random_acc)
        $name_snake_case$.mintWithSig(ITEM1, 1, nonce, sig, {"from": admin, "amount": 15 * 10 ** 17})
...
def test_burn($name_snake_case$, accounts, admin):
    ...
    $name_snake_case$.burn(ITEM1, 1, {"from": accounts[1]})
...
def test_burn_not_enought_tokens($name_snake_case$, accounts, admin):
    sig, nonce = get_mint_sig_with_nonce(accounts[1], ITEM1, 1, admin)
    $name_snake_case$.mintWithSig(ITEM1, 1, nonce, sig, {"from": accounts[1], "amount": 15 * 10 ** 17})
    with brownie.reverts("Not enough tokens"):
        $name_snake_case$.burn(ITEM1, 1, {"from": accounts[2]})

def test_mint_emits_events($name_snake_case$, accounts, admin):
    sig, nonce = get_mint_sig_with_nonce(accounts[5], ITEM1, 2, admin)
    tx = $name_snake_case$.mintWithSig(ITEM1, 2, nonce, sig, {"from": accounts[5], "amount": 15 * 2 * 10 ** 17})
    assert len(tx.events['Mint']) == 1
