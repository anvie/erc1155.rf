...
def test_mint($name_snake_case$, accounts, admin):
    sig, nonce = get_mint_sig_with_nonce(accounts[1], accounts[1], 3, admin)
    tx = $name_snake_case$.mint(accounts[1], 3, nonce, False, sig, {"from": accounts[1], "amount": 15 * 3 * 10 ** 17})

    assert tx.return_value == True
    assert $name_snake_case$.ownerOf(1) == accounts[1]
    assert $name_snake_case$.ownerOf(2) == accounts[1]
    assert $name_snake_case$.ownerOf(3) == accounts[1]
...
def test_mint_airdrop_should_free($name_snake_case$, accounts, admin):
    sig, nonce = get_mint_sig_with_nonce(accounts[1], accounts[1], 1, admin, airdrop=True)
    tx = $name_snake_case$.mint(accounts[1], 1, nonce, True, sig, {"from": accounts[1], "amount": 0})
    assert tx.return_value == True
...
def test_mint_sign_count($name_snake_case$, accounts, admin):
    ...
    with brownie.reverts("invalid signature"):
        $name_snake_case$.mint(accounts[1], 30, nonce, False, sig, {"from": accounts[1], "amount": 15 * 30 * 10 ** 17})


def test_mint_invalid_signature($name_snake_case$, accounts, admin):
    with brownie.reverts("invalid signature"):
        sig, nonce = get_mint_sig_with_nonce(accounts[1], accounts[3], 1, admin)
        $name_snake_case$.mint(accounts[3], 1, nonce, False, sig, {"from": accounts[3], "amount": 15 * 10 ** 17})
    with brownie.reverts("invalid signature"):
        random_acc = accounts.add()
        sig, nonce = get_mint_sig_with_nonce(accounts[1], accounts[3], 1, random_acc)
        $name_snake_case$.mint(accounts[3], 1, nonce, False, sig, {"from": admin, "amount": 15 * 10 ** 17})
...
def test_mint_invalid_nonce($name_snake_case$, accounts, admin):
    with brownie.reverts("invalid nonce"):
        random_acc = accounts.add()
        nonce = get_nonce() - 30
        sig = get_mint_sig(accounts[1], accounts[3], 1, random_acc, nonce)
        $name_snake_case$.mint(accounts[3], 1, nonce, False, sig, {"from": admin, "amount": 15 * 10 ** 17})
...
def test_burn($name_snake_case$, accounts, admin):
    sig, nonce = get_mint_sig_with_nonce(accounts[1], accounts[1], 1, admin)
    $name_snake_case$.mint(accounts[1], 1, nonce, False, sig, {"from": accounts[1], "amount": 15 * 10 ** 17})
...
#             $name_snake_case$.mint(accounts[1], 3, nonce, False, sig, {"from": accounts[1], "amount": 15 * 10 ** 17})
...
def test_mint_emits_events($name_snake_case$, accounts, admin):
    ...
    tx = $name_snake_case$.mint(accounts[5], 2, nonce, False, sig, {"from": accounts[5], "amount": 15 * 2 * 10 ** 17})
